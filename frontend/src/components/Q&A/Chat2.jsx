...
const Chat2 = () => {
  ...
  const askQuestion = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/predict2', { text: userMessage });
      ...
      <Card style={{ width: '100%', maxWidth: 720, borderRadius: 16, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          <div style={{ textAlign: 'center' }}>
            <Title level={3} style={{ color: '#1f3a93', fontWeight: 'bold' }}>✨ AI Chat Asistan Model T5✨</Title>
